...
def delete_confirmation(request, user_id):
    if request.user.is_authenticated:
        user = User.objects.get(id = user_id)
        return render(request, 'admin/delete_verify.html', {"user": user})
...
def averagemarks(request):
    if request.method == "GET":
        students = Student.objects.all()
        return render(request, 'admin/averagemarks.html', {"students":students})
    ...
    if request.method == "POST":
        ...
        assignments =AssignmentSubmit.objects.filter(assignment__due_date__range=[start_from, to_date], student=student)
        is_result = True
        ...
        for assignment in assignments:
            max_marks += assignment.assignment.max_marks
            get_marks += assignment.get_marks
        def get_percent(max_marks, get_marks):
            try:
                return str(float(max_marks) * 100/get_marks)[:5]+ "%"
            except:
                return "0"

        average_percentage =get_percent(max_marks, get_marks)
        ...
        return render(request, 'admin/averagemarks.html', context)
